...
function normalTriangle() {
  // Since we only have one triangle, we call the tri function once
  tri(0, 1, 2, 0);
}
...
function tri(a, b, c, n) {
  ...
  // For 2D, we'll only need one normal vector pointing out of the screen
  var faceNormals = [
    vec4(0.0, 0.0, 1.0, 0.0), // facing towards viewer
  ];

  // Single triangle only needs three indices
  var indices = [a, b, c];
