...
window.onload = function init() {
  ...
  window.addEventListener('keydown', function (e) {
    switch (e.keyCode) {
      ...
      case 57: // 9: Stationary viewpoint from above, looking down at the moving car
        view = 9;
        document.getElementById('Viewpoint').innerHTML =
          '9: Horft á bílinn af húsþaki';
        break;
  ...
  const cycleView = () => {
    view = (view % NUM_VIEWS) + 1;
    document.getElementById('Viewpoint').innerHTML = `${view}: Current view`;
  };
  ...
  canvas.addEventListener('mousemove', function (e) {
    if (isMouseDown) {
      ...
      spinY = (spinY - deltaX * rotationFactor) % 360;
      spinX = (spinX - deltaY * rotationFactor) % 360;

      // Clamp vertical rotation to prevent flipping
      spinX = Math.max(-90, Math.min(90, spinX));
...
function render() {
  ...
  switch (view) {
    ...
    case 9:
      // Stationary viewpoint from above, looking down at the moving car
      mv = lookAt(
        vec3(-40.0, 140.0, 16.0 + height),
        vec3(carXPos, carYPos, 0.0),
        vec3(0.0, 0.0, 1.0)
      );
      drawScenery(mv);
